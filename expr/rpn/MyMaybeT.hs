...
--Normally, Haskell prohibits type signatures on the class method
--implementations in the instance declarations:
...
instance Monad m => Monad (MaybeT m) where
  ...
  (MaybeT ma) >>= f =
    MaybeT $
      ma >>= \case
        --  Uses the LambdaCase GHC extension to do case analysis on the lambda argument
